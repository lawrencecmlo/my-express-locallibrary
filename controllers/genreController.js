...
exports.genre_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        Genre.findByIdAndRemove(results.genre.id, function(err) {
			if (err) { return next(err); };
			res.redirect('/catalog/genres');
		});
    });
...
exports.genre_update_post = [
	...
	body('name').custom(value => {
      return Genre.findOne({ 'name': value }).then(genre => {
        if (genre) {
    ...
    (req, res, next) => {
        ...
        // Update the genre object with escaped and trimmed data.
        var genre = new Genre(
        ...
        return;
            ...
            //         if (found_genre) {
            //             // Genre exists, report error.
			//			 errors.push(new Error('Duplicate genre name'));
            //             res.render('genre_form', { title: 'Update Genre', genre: genre, errors: errors.array()});
            //         }
            //         else {
                         ...
                         Genre.findByIdAndUpdate(req.params.id, genre, {}, function (err,genre) {
								...
								debug("my debug req.body :" + req.body.name);
								res.redirect(genre.url);
							});
